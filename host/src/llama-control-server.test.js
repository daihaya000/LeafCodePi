...
test("/chatgpt-bridge reuses the host control server with typed actions", async () => {
  const calls = [];
  const port = await freePort();
  const server = createLlamaControlServer({
    controlPort: port,
  ...
  await listenControlServer(server, port);
  try {
    const status = await fetch(`http://127.0.0.1:${port}/chatgpt-bridge/status?projectId=project-1`, {
      headers: { host: `127.0.0.1:${port}` },
    });
    assert.equal(status.status, 200);
    assert.deepEqual(await status.json(), { ok: true, action: "status" });

    const enabled = await fetch(`http://127.0.0.1:${port}/chatgpt-bridge/enabled`, {
      method: "POST",
      headers: { host: `127.0.0.1:${port}`, "content-type": "application/json" },
      body: JSON.stringify({ enabled: true }),
    });
    assert.equal(enabled.status, 200);
    assert.deepEqual(await enabled.json(), { ok: true, action: "enabled" });
    assert.deepEqual(calls, [
      { action: "status", body: {}, projectId: "project-1" },
      { action: "enabled", body: { enabled: true }, projectId: null },
    ]);
  } finally {
    await closeControlServer(server);
  }
