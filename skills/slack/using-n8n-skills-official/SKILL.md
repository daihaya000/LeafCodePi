...
> **LeafCodePi note (local addition).** These n8n skills ship as LeafCodePi bundled skills, so the harness is Pi, not a plugin-based client. There is no Skill tool and there are no SessionStart/PreToolUse hooks: when a skill applies, read its `SKILL.md` directly (the path is shown in `available_skills`, for example `skills/n8n-expressions-official/SKILL.md`) before the first n8n action, and read its `references/*.md` files on demand. MCP tools are called through the `mcp` proxy tool: discover with `mcp({ search: "workflow" })`, then call `mcp({ tool: "search_workflows", args: { ... } })`. Tool names and parameters are unchanged. Everything else in this document applies as written.
