...
test("startLoop writeLoop failure does not schedule or claim a started loop", async () => {
  ...
  let sendCount = 0;
  const notifies = [];
  const stateFile = () => join(cwd, "goals-loop", "start-write-fail-session.json");
  ...
  const ctx = {
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "start-write-fail-session",
      getBranch: () => [],
    },
    ui: {
      setStatus: () => {},
...
test("does not retain initial images when post-send cleanup persistence fails", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "leafcode-goal-loop-initial-image-cleanup-"));
  process.env.LEAFCODE_PI_DATA_DIR = cwd;
  const handlers = new Map();
  const commands = new Map();
  const sent = [];
  let busy = false;
  const stateFile = () => join(cwd, "goals-loop", "initial-image-cleanup-session.json");
  const ctx = {
    cwd,
    mode: "rpc",
    hasUI: false,
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "initial-image-cleanup-session",
      getBranch: () => [],
    },
    ui: { setStatus: () => {}, setWidget: () => {}, notify: () => {} },
  };
  ...
  try {
    ...
    await handlers.get("session_start")?.({}, ctx);
    // startLoop and the pre-send running snapshot succeed; the best-effort
    // cleanup write after sendMessage fails.
    goalLoopTestSeams.setWriteLoopAllowCount(2);
    const payload = Buffer.from(JSON.stringify({
      goal: "画像を確認する",
      maxTurns: 1,
      images: [{ mimeType: "image/png", data: "aW1hZ2U=" }],
    ...
    assert.equal("initialImages" in persisted, false);
  } finally {
    goalLoopTestSeams.setWriteLoopAllowCount();
    await handlers.get("session_shutdown")?.({}, ctx);
    rmSync(cwd, { recursive: true, force: true });
  }
});
...
test("resume recovery writeLoop failure does not double-apply transcript JSON", async () => {
  ...
  let busy = false;
  let sendCount = 0;
  const stateFile = () => join(cwd, "goals-loop", "resume-write-fail-session.json");
  const branch = [];
  ...
  const ctx = {
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "resume-write-fail-session",
      getBranch: () => branch,
    },
    ui: { setStatus: () => {}, setWidget: () => {}, notify: () => {} },
  };
  ...
  try {
    ...
    // Mid-turn pause leaves durable recovery; transcript already has the JSON.
    await commands.get("goal-pause")?.("", ctx);
    branch.push({
      type: "message",
      message: {
        role: "assistant",
        content: [{ type: "text", text: JSON.stringify({ status: "progress", summary: "once only" }) }],
      },
    });
    const paused = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(paused.status, "paused");
    ...
    assert.equal(failed.progress.length, 0);

    goalLoopTestSeams.setWriteLoopFail(false);
    await commands.get("goal-resume")?.("", ctx);
    const recovered = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(recovered.pendingTurnRecovery, false);
    assert.equal(recovered.progress.filter((item) => item.summary === "once only").length, 1);

    // A third resume must not re-apply the same transcript JSON.
    if (recovered.status === "paused") {
      await commands.get("goal-resume")?.("", ctx);
    }
    const again = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(again.progress.filter((item) => item.summary === "once only").length, 1);
  } finally {
    goalLoopTestSeams.setWriteLoopFail(false);
    await handlers.get("session_shutdown")?.({}, ctx);
    rmSync(cwd, { recursive: true, force: true });
...
test("mid-turn pause recovers JSON from agent_settled after pause clears awaitingTurn", async () => {
  ...
  let busy = false;
  let sendCount = 0;
  const stateFile = () => join(cwd, "goals-loop", "manual-settled-session.json");
  ...
  const ctx = {
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "manual-settled-session",
      getBranch: () => [],
    },
    ui: { setStatus: () => {}, setWidget: () => {}, notify: () => {} },
  };
  ...
  try {
    ...
    await handlers.get("agent_end")?.({
      ...
      messages: [{
        role: "assistant",
        content: [{ type: "text", text: JSON.stringify({ status: "progress", summary: "from agent_settled" }) }],
      }],
    }, ctx);
    await handlers.get("agent_settled")?.({ type: "agent_settled" }, ctx);
...
test("mid-turn pause late result keeps progress when re-pause write fails", async () => {
  ...
  let sendCount = 0;
  const notifies = [];
  const stateFile = () => join(cwd, "goals-loop", "repause-write-fail-session.json");
  ...
  const ctx = {
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "repause-write-fail-session",
      getBranch: () => [],
    },
    ui: {
      setStatus: () => {},
  ...
  try {
    ...
    await handlers.get("agent_end")?.({
      ...
      messages: [{
        role: "assistant",
        content: [{ type: "text", text: JSON.stringify({ status: "progress", summary: "kept despite repause fail" }) }],
      }],
    }, ctx);
    await handlers.get("agent_settled")?.({ type: "agent_settled" }, ctx);

    const loop = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(loop.progress.at(-1)?.summary, "kept despite repause fail");
    // Re-pause write failed: disk stays queued (progress durable) and must not auto-send.
    assert.equal(loop.status, "queued");
    assert.ok(notifies.some((item) => item.level === "error" && /一時停止状態の保存に失敗/.test(item.message)));
    await new Promise((resolve) => setTimeout(resolve, 700));
    assert.equal(sendCount, 1);
  } finally {
    goalLoopTestSeams.setWriteLoopAllowCount(undefined);
    await handlers.get("session_shutdown")?.({}, ctx);
    rmSync(cwd, { recursive: true, force: true });
  }
});

test("resume budget and unknown_delivery paths honor writeLoop failure", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "leafcode-goal-loop-resume-paths-write-fail-"));
  process.env.LEAFCODE_PI_DATA_DIR = cwd;
  mkdirSync(join(cwd, "goals-loop"), { recursive: true });
  const handlers = new Map();
  const commands = new Map();
  const notifies = [];
  const stateFile = () => join(cwd, "goals-loop", "resume-paths-write-fail-session.json");
  const now = new Date().toISOString();
  ...
  const writeState = (patch) => {
    const current = existsSync(stateFile())
      ...
      : {
        id: "resume-paths-write-fail-session",
        sessionId: "resume-paths-write-fail-session",
        cwd,
        goal: "demo",
        acceptance: ["ok"],
        cooldownSeconds: 0,
    ...
    writeFileSync(stateFile(), JSON.stringify({ ...current, ...patch, updatedAt: now }, null, 2));
  };
  ...
  const ctx = {
    ...
    abort: () => {},
    sessionManager: {
      getSessionId: () => "resume-paths-write-fail-session",
      getBranch: () => [],
    },
    ui: {
      setStatus: () => {},
  ...
  try {
    ...
    goalLoopTestSeams.setWriteLoopFail(true);
    notifies.length = 0;
    await commands.get("goal-resume")?.("--turns 1", ctx);
    const budget = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(budget.status, "paused");
    assert.equal(budget.pauseReason, "turn_limit");
    assert.equal(budget.maxTurns, 1);
    assert.ok(notifies.some((item) => item.level === "error" && /状態の保存に失敗/.test(item.message)));
    ...
    notifies.length = 0;
    await commands.get("goal-resume")?.("", ctx);
    const unknown = JSON.parse(readFileSync(stateFile(), "utf8"));
    assert.equal(unknown.status, "paused");
    assert.equal(unknown.pauseReason, "unknown_delivery");
    assert.equal(unknown.pendingTurnRecovery, true);
    assert.ok(notifies.some((item) => item.level === "error" && /状態の保存に失敗/.test(item.message)));
  } finally {
    goalLoopTestSeams.setWriteLoopFail(false);
    await handlers.get("session_shutdown")?.({}, ctx);
...
test("late turn_end with a later turnIndex still recovers after mid-turn pause", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "leafcode-goal-loop-turn-index-"));
  process.env.LEAFCODE_PI_DATA_DIR = cwd;
  const handlers = new Map();
  const commands = new Map();
  let busy = false;
  let sendCount = 0;
  const stateFile = () => join(cwd, "goals-loop", "turn-index-session.json");
  ...
  const ctx = {
    ...
    abort: () => { busy = false; },
    sessionManager: {
      getSessionId: () => "turn-index-session",
      getBranch: () => [],
    },
    ui: { setStatus: () => {}, setWidget: () => {}, notify: () => {} },
  };
  ...
  try {
    ...
    await handlers.get("turn_end")?.({
      ...
      message: {
        role: "assistant",
        content: [{ type: "text", text: JSON.stringify({ status: "progress", summary: "later turnIndex" }) }],
      },
    }, ctx);
