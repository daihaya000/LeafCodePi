...
const SubagentParamProperties = {
	...
	isolation: Type.Optional(Type.String({ enum: ["none", "worktree"], description: "Workflow child isolation. none runs in the shared cwd; worktree uses a separate managed worktree." })),
	worktree: Type.Optional(Type.Boolean({ description: "Managed child isolation. true uses a separate managed worktree; shared checkout runs use false or omit this field." })),
