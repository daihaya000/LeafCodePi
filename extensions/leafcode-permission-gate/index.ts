...
function isReadOnlyDiskInspection(command: string): boolean {
  ...
  if (/\b(?:mkfs(?:\.\w+)?|fdisk|sfdisk|parted|cfdisk|sgdisk|wipefs|diskpart|diskutil|mdadm|swapoff|mkswap|blkdiscard|nvme|sg_format|sg_sanitize|badblocks|hdparm|smartctl|sdelete|defrag|storcli|perccli|MegaCli|fsutil|format(?:\.com|\.exe)?|(?:Format|Resize|New|Remove|Set|Optimize)-Volume|vssadmin|cipher)\b[^\r\n]*(?:\/\?|--help|-h\b|-\?)/i.test(normalized)
  ...
  return [
    ...
    /^diskpart(?:\.exe)?\s+list\s+(?:disk|volume|partition)\b/i,
    /^nvme(?:-cli)?\s+list(?:\s+\S+)*$/i,
    /^sg_format\b[^\r\n]*(?:--inquiry|--version|-i\b)/i,
    /^diskpart(?:\.exe)?\s+select\s+(?:disk|volume|partition)\b/i,
