...
const SYSTEM_SAFETY_RULES: readonly SystemSafetyRule[] = [
  ...
  { category: "os", label: "system policy/account/firewall change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Set-ExecutionPolicy|setx|icacls|takeown|net(?:\.exe)?\s+(?:user|localgroup)|(?:New|Remove|Add|Disable|Enable)-Local(?:User|GroupMember)|(?:New|Set|Remove)-(?:NetFirewallRule|WindowsOptionalFeature)|(?:Enable|Disable)-WindowsOptionalFeature)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)dism(?:\.exe)?\b[^\r\n]*\/(?:enable-feature|disable-feature|add-package|remove-package)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)msiexec(?:\.exe)?\b[^\r\n]*\/(?:i|uninstall)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)netsh(?:\.exe)?\b[^\r\n]*\badvfirewall\b[^\r\n]*\bset\b[^\r\n]*\bstate\s+off\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)networksetup\b[^\r\n]*-(?:setairportpower|setnetworkserviceenabled)\b[^\r\n]*\boff\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)airport\b[^\r\n]*-z\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)spctl\b[^\r\n]*--(?:master-disable|global-disable|disable-assessment)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)Set-NetFirewallProfile\b[^\r\n]*-Enabled\s+(?:False|0)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)ufw\b[^\r\n]*\bdisable\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)firewall-cmd\b[^\r\n]*--(?:panic-on|set-default-zone=drop)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:iptables|ip6tables)\b[^\r\n]*-P\s+\S+\s+DROP\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:iptables|ip6tables)\b[^\r\n]*\s-[FX]\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)nft\b[^\r\n]*\b(?:flush\s+ruleset|delete\s+(?:table|chain|rule)\b)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)pfctl\b[^\r\n]*(?:\s-[dDF]\b|\s-F\s+all\b)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)setenforce\s+(?:0|[Pp]ermissive)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)aa-(?:disable|complain|remove|teardown)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)apparmor_parser\b[^\r\n]*\s-R\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)profiles\b[^\r\n]*\bremove\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)security\b[^\r\n]*\bauthorizationdb\b[^\r\n]*\b(?:write|remove)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)dscl\b[^\r\n]*-delete\b[^\r\n]*\/Users\/|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)sysadminctl\b[^\r\n]*-(?:deleteUser|secureTokenOff)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:userdel|deluser)\b/i },
  { category: "os", label: "system package change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:[A-Za-z]:[\\/])?(?:[\\/]*(?:[\w.-]+[\\/])*)?(?:apt(?:-get)?|dnf|yum|pacman|zypper|apk|brew|winget|choco)(?:\.exe)?\b[^\r\n]*(?:install|remove|purge|upgrade|update|add|delete|uninstall|-[SRU][A-Za-z]*)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:npm|pnpm|yarn|pip|pip3)(?:\.exe)?\b[^\r\n]*\b(?:install|add|remove|uninstall|update|i|un|rm|up)\b[^\r\n]*(?:--global|\s-g\b)/i },
  { category: "os", label: "scheduled task change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Register|Unregister|New|Remove)-ScheduledTask\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)schtasks(?:\.exe)?\b[^\r\n]*\/(?:create|delete|change|run)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*crontab(?:\.exe)?(?:\s+-u\s+\S+)?\s+(?!-(?:l|h)\b|--(?:help|version)\b)(?:-\w|\S)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*\bat(?:\.exe)?\b[^\r\n]*(?:\s-f\s|\s(?:now|midnight|noon)\b|\d{1,2}:\d{2})|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:echo|printf|print)\b[^|\r\n]*\|\s*(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*\bbatch\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)anacron\b[^\r\n]*-(?:f|n|d)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?systemctl\s+--user\s+(?:enable|mask|link|preset)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?loginctl\b[^\r\n]*\b(?:enable|disable)-linger\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?launchctl\s+(?:bootstrap|bootout|load|unload|enable|disable)\b/i },
  // AV / endpoint disable is machine-breaking at standard (not mere policy tweak).
  { category: "os", label: "security software disable", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*Set-MpPreference\b[^\r\n]*-Disable(?:RealtimeMonitoring|BehaviorMonitoring|IOAVProtection|ScriptScanning|BlockAtFirstSeen|IntrusionPreventionSystem|TamperProtection|OnAccessProtection)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*Add-MpPreference\b[^\r\n]*-Exclusion(?:Path|Process|Extension|IpAddress)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*Remove-MpPreference\b[^\r\n]*-ThreatID|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*(?:Disable-WindowsDefender|(?:Uninstall|Remove)-WindowsFeature\b[^\r\n]*\bWindows-Defender|Remove-WindowsCapability\b[^\r\n]*Windows\.?Defender)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*Set-MpPreference\b[^\r\n]*-EnableControlledFolderAccess\s+Disabled\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*sc(?:\.exe)?\s+(?:stop|config)\s+WinDefend\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*(?:Stop|Set)-Service\b[^\r\n]*\bWinDefend\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*systemctl\s+(?:stop|disable|mask)\s+(?:clamav(?:-daemon|-freshclam)?|falcon-sensor|mdatp|sentineld|cbdaemon|csf)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*service\s+(?:clamav-daemon|falcon-sensor|mdatp)\s+stop\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+|[\w.]+=\S+))*\s+)*chkconfig\s+(?:clamav(?:-daemon)?|falcon-sensor|mdatp)\s+off\b/i },
  ...
  { category: "service", label: "service/daemon change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)sc(?:\.exe)?\s+(?:create|config|delete|start|stop|failure|privs)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:New|Remove|Set|Start|Stop|Restart)-(?:Windows)?Service\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)systemctl(?:\s+--user)?\s+(?:enable|disable|start|stop|restart|mask|unmask|link|preset)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)service\s+\S+\s+(?:start|stop|restart)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:launchctl\s+(?:load|unload|bootstrap|bootout|enable|disable)|rc-service\s+\S+\s+(?:start|stop|restart))\b|\b(?:start|stop|restart|enable|disable)\s+(?:the\s+)?(?:service|daemon)s?\b|\b(?:start|stop|restart|enable|disable)[_-](?:service|daemon)s?\b/i },
  // Require bootloader/configuration — bare "update boot" is a commit message / package name.
  { category: "boot", label: "boot configuration change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:bcdboot(?:\.exe)?|grub-install|update-grub|update-initramfs)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)bootrec(?:\.exe)?\b[^\r\n]*\/(?:fixmbr|fixboot|rebuildbcd)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)efibootmgr\b[^\r\n]*(?:\s-[cCbBdDoOnN]|--(?:create|delete|disk|bootorder|bootnext))\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)reagentc(?:\.exe)?\b[^\r\n]*\/(?:enable|disable|setreimage|boottore)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)bootcfg(?:\.exe)?\b[^\r\n]*\/(?:add|delete|raw)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)bcdedit(?:\.exe)?\b[^\r\n]*[\/-](?:set|delete(?:value)?|create|import|export|store|timeout|default|displayorder|bootsequence|ems|dbgsettings|hypervisorsettings)\b|\b(?:change|modify|update|repair|write|set)\s+(?:the\s+)?boot(?:loader|configuration)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)csrutil\b[^\r\n]*\b(?:disable|clear|enable)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)nvram\b[^\r\n]*(?:\s-[cd]\b|--delete|\s+\S+=)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)bless\b[^\r\n]*--(?:setBoot|bootinfo|netboot)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)mokutil\b[^\r\n]*--(?:disable-validation|enable-validation|reset|revoke-import|import)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)bootctl\b[^\r\n]*\b(?:set-default|set-oneshot|unset-oneshot|install)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)systemd-boot\b[^\r\n]*\binstall\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)sbsign\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:grub-set-default|grub-reboot|update-grub2|grub2-mkconfig|kernel-install|bootsect)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)grub-editenv\b[^\r\n]*\bset\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)dracut\b[^\r\n]*--(?:force|regenerate-all)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Set-SecureBootUEFI|Confirm-SecureBootUEFI|update-secureboot-policy(?:\.exe)?)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:tpm2_clear(?:control)?|Clear-Tpm|Initialize-Tpm)\b/i },
  // `format C:` / mkfs / diskpart / *-Volume require command position — not `echo mkfs` / `Get-Command Format-Volume`.
  { category: "disk", label: "disk/partition/volume change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|call|start|stdbuf|setsid)(?:\s+(?!-[vV]\b|--(?:help|version)\b)--?[\w.-]+(?:=\S+)?|\s+--(?=\s)|\s+\/[A-Za-z]+\b|\s+""|\s+\d+|\s+(?!-[vV]\b)-\w+\s+\S+|\s+(?!(?:[\\/]*(?:[\w.-]+[\\/])*)?(?:shutdown|reboot|poweroff|halt|sudo|doas|pkexec|runas|gsudo|su)(?:\.exe)?(?![\w-]))(?:(?:~\/?|%[\w]+%|\$\{?[\w:]+\}?|\.\.?\/|[A-Za-z]:[\\/]|\/)[A-Za-z0-9_./\\:%~${}$-]*|[A-Za-z_][\w.@-]*))*\s+)*(?:[A-Za-z]:[\\/])?(?:[\\/]*(?:[\w.-]+[\\/])*)?(?:mkfs(?:\.\w+)?|fdisk|sfdisk|parted|cfdisk|sgdisk|wipefs|diskpart(?:\.exe)?|diskutil)(?:\.exe)?\b|\bdd\b[^\r\n]*\bof=(?:\/(?:dev|etc|boot|sys|proc|usr|var|opt|root|sbin|bin|lib|private)\/|\\\\\.\\|[A-Za-z]:[\\/])|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|call|start|stdbuf|setsid)(?:\s+(?!-[vV]\b|--(?:help|version)\b)--?[\w.-]+(?:=\S+)?|\s+--(?=\s)|\s+\/[A-Za-z]+\b|\s+""|\s+\d+|\s+(?!-[vV]\b)-\w+\s+\S+|\s+(?!(?:[\\/]*(?:[\w.-]+[\\/])*)?(?:shutdown|reboot|poweroff|halt|sudo|doas|pkexec|runas|gsudo|su)(?:\.exe)?(?![\w-]))(?:(?:~\/?|%[\w]+%|\$\{?[\w:]+\}?|\.\.?\/|[A-Za-z]:[\\/]|\/)[A-Za-z0-9_./\\:%~${}$-]*|[A-Za-z_][\w.@-]*))*\s+)*(?:[A-Za-z]:[\\/])?(?:[\\/]*(?:[\w.-]+[\\/])*)?(?:format(?:\.com|\.exe)\b|format(?:\s+[\/\-][A-Za-z0-9:]+)*\s+[A-Za-z]:(?:\s|$|\/))|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Clear|Initialize|Set|New|Remove)-(?:Disk|Partition|Volume)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Format|Resize|New|Remove|Set)-Volume\b|\b(?:format|erase|wipe|partition|resize|initialize)\s+(?:the\s+)?(?:disk|drive|volume|partition)s?\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)vssadmin(?:\.exe)?\b[^\r\n]*\bdelete\b[^\r\n]*\bshadows\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)cipher(?:\.exe)?\b[^\r\n]*\/w\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Disable-BitLocker|Enable-BitLocker)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)manage-bde(?:\.exe)?\b[^\r\n]*-(?:off|on|protectors\s+(?:delete|add)|forcerecovery)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)fdesetup\b[^\r\n]*\b(?:disable|removepassword|erase|changerecovery)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)mdadm\b[^\r\n]*--(?:stop|zero-superblock|remove|fail|create|grow|manage)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:vgremove|lvremove|pvremove)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?cryptsetup\b[^\r\n]*\b(?:luksFormat|luksErase|erase|luksAddKey|luksRemoveKey|luksKillSlot|luksChangeKey)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?zfs\b[^\r\n]*\b(?:destroy|rollback)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?zpool\b[^\r\n]*\bdestroy\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?btrfs\b[^\r\n]*\bsubvolume\b[^\r\n]*\bdelete\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?xfs_repair\b[^\r\n]*-L\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*(?:swapoff|mkswap|blkdiscard)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*nvme(?:-cli)?\b[^\r\n]*\b(?:format|sanitize|delete-ns)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*sg_format\b[^\r\n]*(?:--format|-F\b)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*badblocks\b[^\r\n]*-w\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:Reset|Clear)-PhysicalDisk\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?Clear-FileStorageTier\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*hdparm\b[^\r\n]*(?:--(?:security-set-pass|user-master-password|make-bad-sector|security-erase(?:-enhanced|-prompt)?)|-E\b)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*(?:storcli(?:64)?|perccli(?:64)?|MegaCli(?:64)?)(?:\.exe)?\b[^\r\n]*\b(?:erase|secureerase|PDClear|CfgLdDel|init\s+full|del\s+force)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?fsutil(?:\.exe)?\b[^\r\n]*\bvolume\b[^\r\n]*\bformat\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*smartctl\b[^\r\n]*--sanitize\b(?!\s+status\b)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*sg_sanitize\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:Remove-StoragePool|Remove-VirtualDisk)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?Optimize-Volume\b[^\r\n]*-WipeFreeSpace\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?defrag(?:\.exe)?\b[^\r\n]*\/[Ww]\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?sdelete(?:\.exe)?\b[^\r\n]*-[zc]\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*camcontrol\b[^\r\n]*\bformat\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*gpart\b[^\r\n]*\b(?:destroy|delete|create|add)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*newfs(?:_[\w]+|\.[\w]+)?\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*diskshadow(?:\.exe)?\b[^\r\n]*\/[sS]\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:Remove|New|Resize)-VHD\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*geli\b[^\r\n]*\b(?:init|attach|detach|setkey|kill|onetime)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*bsdlabel\b[^\r\n]*-[wR]\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*glabel\b[^\r\n]*\b(?:destroy|create)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*(?:arcconf|ssacli|hpacucli)(?:\.exe)?\b[^\r\n]*\b(?:DELETE|delete|erase|forced|SETSTATE|modify\s+erase)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*geom\b[^\r\n]*\b(?:destroy|delete|wipe)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*(?:growfs|tunefs|tune2fs|resize2fs|xfs_growfs|lvresize|pvresize|lvextend|lvreduce|lvcreate|pvcreate|vgcreate|lvchange|vgchange)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*dmsetup\b[^\r\n]*\bremove\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*losetup\b[^\r\n]*-d\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*camcontrol\b[^\r\n]*\beject\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:[\w.]+=\S+\s+)*)?(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf|ssh(?:\.exe)?|docker(?:\.exe)?|podman(?:\.exe)?|wsl(?:\.exe)?|xargs|flock|parallel|systemd-run)(?:\s+(?:(?!-[vV]\b|--(?:help|version)\b)-\S+|\d+|[\w.]+=\S+|[A-Za-z0-9_@.:/%\\-]+))*\s+)*cryptsetup\b[^\r\n]*\b(?:close|luksClose|open|luksOpen)\b/i },
  // Require flash/write/the firmware — bare "npm install firmware" is a package name.
  { category: "firmware", label: "firmware/BIOS update", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)fwupdmgr\b[^\r\n]*\b(?:install|update|refresh)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)flashrom\b[^\r\n]*(?:-w|--write|\bwrite\b)|\b(?:flash|update|write|set)[ -]*(?:bios|uefi)\b|\b(?:flash|write)\s+(?:the\s+)?firmware\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Update|Set|Write)-Firmware\b|\b(?:flash|update|write|install|erase)\s+the\s+(?:firmware|bios|uefi)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)firmwarepasswd\b[^\r\n]*-(?:delete|setpasswd)\b/i }
];
...
function isLowIntensityMatch(match: SystemSafetyMatch): boolean {
  ...
  if (match.category === "os") {
    return match.label === "OS shutdown/restart"
      ...
      || match.label === "scheduled task change"
      || match.label === "security software disable";
  }
  return false;
}
...
function isReadOnlyDiskInspection(command: string): boolean {
  ...
  if (/\b(?:mkfs(?:\.\w+)?|fdisk|sfdisk|parted|cfdisk|sgdisk|wipefs|diskpart|diskutil|mdadm|swapoff|mkswap|blkdiscard|nvme|sg_format|sg_sanitize|badblocks|hdparm|smartctl|sdelete|defrag|storcli|perccli|MegaCli|fsutil|camcontrol|gpart|newfs|diskshadow|geli|bsdlabel|glabel|arcconf|ssacli|hpacucli|geom|growfs|tunefs|tune2fs|resize2fs|dmsetup|losetup|format(?:\.com|\.exe)?|(?:Format|Resize|New|Remove|Set|Optimize)-Volume|vssadmin|cipher)\b[^\r\n]*(?:\/\?|--help|-h\b|-\?)/i.test(normalized)
    && (/(?:\/\?|--help|-h\b|-\?)\s*$/i.test(normalized) || !/\s+[A-Za-z]:(?:\s|$|\/)/.test(normalized))
    && (!/\/w\b/i.test(normalized) || /(?:\/\?|--help|-h\b|-\?)\s*$/i.test(normalized))
  ...
  return [
    ...
    /^diskutil\s+verify(?:Volume|Disk|Container)\s+\S+$/i,
    /^mdadm\s+(?:--detail|-D|--examine|-E|--query|-Q)(?:\s+\S+)*$/i,
    /^diskpart(?:\.exe)?\s+list\s+(?:disk|volume|partition|vdisk)\b/i,
    /^newfs(?:_[\w]+|\.[\w]+)?\b[^\r\n]*\s-N\b/i,
    /^zfs\b[^\r\n]*\bdestroy\b[^\r\n]*-[a-zA-Z]*n/i,
    /^zpool\b[^\r\n]*\bdestroy\b[^\r\n]*-[a-zA-Z]*n/i,
    /^losetup\b(?:\s+-a|\s+--list)?\s*$/i,
    /^command\s+-[vV]\s+\S+$/i,
    /^(?:growfs|tunefs|tune2fs|resize2fs|xfs_growfs|lvresize|pvresize|lvextend|lvreduce)\b[^\r\n]*(?:\/\?|--help|-h\b|-\?)\s*$/i,
    /^camcontrol\s+(?:rescan|inquiry)\b/i,
    /^diskshadow(?:\.exe)?\s+(?:\/l\b|list\b)/i,
    /^geli\s+list(?:\s+\S+)*$/i,
    /^glabel\s+status(?:\s+\S+)*$/i,
    /^nvme(?:-cli)?\s+list(?:\s+\S+)*$/i,
    /^sg_format\b[^\r\n]*(?:--inquiry|--version|-i\b)/i,
    /^diskpart(?:\.exe)?\s+select\s+(?:disk|volume|partition|vdisk)\b/i,
...
/** `echo …` / `Write-Host …` with no unquoted shell operators or redirects — do not unwrap or path-gate. */
function isDocumentationOnlyCommand(command: string): boolean {
  ...
  // Redirects mutate files (`echo x > /etc/cron.d/evil`) — not documentation.
  return !/[;&|]|(?<![0-9])>{1,2}/.test(unquoted);
}
