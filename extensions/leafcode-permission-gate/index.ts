...
function isReadOnlyDiskInspection(command: string): boolean {
  ...
  if (/\b(?:mkfs(?:\.\w+)?|fdisk|sfdisk|parted|cfdisk|sgdisk|wipefs|diskpart|diskutil|mdadm|swapoff|mkswap|blkdiscard|nvme|sg_format|format(?:\.com|\.exe)?|(?:Format|Resize|New|Remove|Set)-Volume|vssadmin|cipher)\b[^\r\n]*(?:\/\?|--help|-h\b|-\?)/i.test(normalized)
