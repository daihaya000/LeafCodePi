...
function isReadOnlyDiskInspection(command: string): boolean {
  ...
  if (/\b(?:mkfs(?:\.\w+)?|fdisk|sfdisk|parted|cfdisk|sgdisk|wipefs|diskpart|diskutil|mdadm|swapoff|mkswap|blkdiscard|nvme|sg_format|sg_sanitize|badblocks|hdparm|smartctl|sdelete|defrag|storcli|perccli|MegaCli|fsutil|camcontrol|gpart|newfs|diskshadow|format(?:\.com|\.exe)?|(?:Format|Resize|New|Remove|Set|Optimize)-Volume|vssadmin|cipher)\b[^\r\n]*(?:\/\?|--help|-h\b|-\?)/i.test(normalized)
