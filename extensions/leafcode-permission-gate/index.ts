...
const SYSTEM_SAFETY_RULES: readonly SystemSafetyRule[] = [
  ...
  { category: "os", label: "scheduled task change", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:Register|Unregister|New|Remove)-ScheduledTask\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)schtasks(?:\.exe)?\b[^\r\n]*\/(?:create|delete|change|run)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*crontab(?:\.exe)?(?:\s+-u\s+\S+)?\s+(?!-(?:l|h)\b|--(?:help|version)\b)(?:-\w|\S)|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*\bat(?:\.exe)?\b[^\r\n]*(?:\s-f\s|\s(?:now|midnight|noon)\b|\d{1,2}:\d{2})|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:echo|printf|print)\b[^|\r\n]*\|\s*(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)\s+)*\bbatch\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)anacron\b[^\r\n]*-(?:f|n|d)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)systemctl\s+--user\s+(?:enable|mask|link|preset)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)loginctl\b[^\r\n]*\b(?:enable|disable)-linger\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)launchctl\s+(?:bootstrap|bootout|load|unload|enable|disable)\b/i },
  // AV / endpoint disable is machine-breaking at standard (not mere policy tweak).
  { category: "os", label: "security software disable", pattern: /(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*Set-MpPreference\b[^\r\n]*-Disable(?:RealtimeMonitoring|BehaviorMonitoring|IOAVProtection|ScriptScanning|BlockAtFirstSeen|IntrusionPreventionSystem|TamperProtection|OnAccessProtection)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*Add-MpPreference\b[^\r\n]*-Exclusion(?:Path|Process|Extension|IpAddress)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*(?:Disable-WindowsDefender|(?:Uninstall|Remove)-WindowsFeature\b[^\r\n]*\bWindows-Defender|Remove-WindowsCapability\b[^\r\n]*Windows\.?Defender)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*Set-MpPreference\b[^\r\n]*-EnableControlledFolderAccess\s+Disabled\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*sc(?:\.exe)?\s+(?:stop|config)\s+WinDefend\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*(?:Stop|Set)-Service\b[^\r\n]*\bWinDefend\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*systemctl\s+(?:stop|disable|mask)\s+(?:clamav(?:-daemon|-freshclam)?|falcon-sensor|mdatp|sentineld|cbdaemon|csf)\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*service\s+(?:clamav-daemon|falcon-sensor|mdatp)\s+stop\b|(?:^|[;&|\r\n]\s*|&&\s*|\|\|\s*)(?:(?:env|busybox|timeout|exec|nohup|nice|command|time|stdbuf)(?:\s+(?:-\S+|\d+))?\s+)*chkconfig\s+(?:clamav(?:-daemon)?|falcon-sensor|mdatp)\s+off\b/i },
