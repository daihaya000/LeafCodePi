...
function printHelp(log = console.log) {
  log("leafcode-mcp-adapter helper\n");
  log("Install the package with:");
  log("  pi install ./extensions/leafcode-mcp-adapter\n");
  log("Then optionally run:");
  log("  leafcode-mcp-adapter init       Detect host configs and scaffold Pi imports");
  log("  leafcode-mcp-adapter init --dry-run");
  log("  leafcode-mcp-adapter init --discover-host-configs  Opt in to host config fallback discovery");
  log("");
  log("Bearer token storage (servers configured with auth: \"bearer\" and bearerTokenStore: true):");
  log("  leafcode-mcp-adapter token set <server>     Store a token read from stdin (masked prompt or pipe; never argv)");
  log("  leafcode-mcp-adapter token status <server>  Report whether a stored token matches the configured URL");
  log("  leafcode-mcp-adapter token remove <server>  Remove the stored token");
}
...
async function runToken(argv, log, error, stdin) {
  ...
  if (!["set", "status", "remove"].includes(action) || !serverName) {
    error("Usage: leafcode-mcp-adapter token <set|status|remove> <server>");
  ...
  if (action === "status") {
    ...
    if (status.status === "url-mismatch") {
      log(`Bearer token is stored for "${safeName}", but its URL does not match the current server URL. Run \`leafcode-mcp-adapter token set ${safeName}\` to rebind it.`);
      return 1;
    }
...
export async function main(argv = process.argv.slice(2), log = console.log, error = console.error, stdin = process.stdin) {
  ...
  if (command === "install") {
    ...
    error("Use `pi install ./extensions/leafcode-mcp-adapter` instead, then optionally run `leafcode-mcp-adapter init`.");
    return 1;
  }
