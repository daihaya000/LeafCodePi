...
export async function manageBearerToken(
  ...
  if (action === "set") {
    const message = `Cannot store bearer token here: Pi extension UI has no masked secret input primitive. Run \`leafcode-mcp-adapter token set ${safeName}\` in a terminal; it reads the token from stdin only.`;
    if (ui) ui.notify(message, "error");
    return { ok: false, message };
  }
