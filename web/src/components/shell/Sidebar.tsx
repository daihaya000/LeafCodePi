  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        ...
        <div className="mt-1 space-y-1">{visibleRooms.map((room) => <button key={room.id} type="button" onClick={() => { router.push(`/bots/rooms/${room.id}`); onClose(); }} className={`relative flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-surface-2 ${pathname === `/bots/rooms/${room.id}` ? "bg-surface-2 before:absolute before:inset-y-1 before:-left-2 before:w-0.5 before:rounded-full before:bg-accent" : ""}`}><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-success-bg text-xs text-success">#</span><span className="min-w-0 flex-1"><span className="block truncate text-sm">{room.name}</span><span className="block truncate text-[11px] text-muted">{room.lastMessageSummary ?? "\u30e1\u30c3\u30bb\u30fc\u30b8\u306a\u3057"}</span></span>{pathname !== `/bots/rooms/${room.id}` && hasUnread(room.lastMessageAt, getLastReadAt("room", room.id)) && <span aria-label={"\u672a\u8aad"} title={"\u672a\u8aad"} className="h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />}<span className="shrink-0 self-start pt-0.5 text-[10px] text-muted">{timeAgo(room.lastMessageAt ?? room.updatedAt)}</span></button>)}{visibleRooms.length === 0 && <p className="px-2 py-2 text-xs text-muted">\u30eb\u30fc\u30e0\u306f\u3042\u308a\u307e\u305b\u3093</p>}</div>
        <div className="sticky top-0 z-10 mt-5 flex items-center justify-between bg-surface px-2 py-1"><span className="text-xs font-medium text-muted">Bot</span><Link href="/bots" onClick={onClose} className="text-xs text-accent">すべて</Link></div>
        <div className="mt-1 space-y-1">{visibleBots.map((bot) => <button key={bot.id} type="button" onClick={() => { router.push(`/bots/${bot.id}`); onClose(); }} className={`relative flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-surface-2 ${pathname === `/bots/${bot.id}` ? "bg-surface-2 before:absolute before:inset-y-1 before:-left-2 before:w-0.5 before:rounded-full before:bg-accent" : ""}`}><BotAvatar size={32} color={bot.avatarColor} image={bot.avatarImage} name={bot.name} /><span aria-label={bot.enabled ? "\u6709\u52b9" : "\u7121\u52b9"} title={bot.enabled ? "\u6709\u52b9" : "\u7121\u52b9"} className={`h-1.5 w-1.5 shrink-0 rounded-full ${bot.enabled ? "bg-success" : "bg-muted"}`} /><span className="min-w-0 flex-1"><span className="block truncate text-sm">{bot.name}</span><span className="block truncate text-[11px] text-muted">{bot.lastMessageSummary ?? "\u30e1\u30c3\u30bb\u30fc\u30b8\u306a\u3057"}</span></span>{pathname !== `/bots/${bot.id}` && hasUnread(bot.lastMessageAt, getLastReadAt("bot", bot.id)) && <span aria-label={"\u672a\u8aad"} title={"\u672a\u8aad"} className="h-1.5 w-1.5 shrink-0 rounded-full bg-accent" />}<span className="shrink-0 self-start pt-0.5 text-[10px] text-muted">{bot.lastMessageAt ? timeAgo(bot.lastMessageAt) : ""}</span></button>)}{visibleBots.length === 0 && <p className="px-2 py-2 text-xs text-muted">Bot\u306f\u3042\u308a\u307e\u305b\u3093</p>}</div>
      </div>
