...
function ContextUsageMeter({ usage }: { usage: ContextUsageDto }) {
  ...
  return (
    <span
      className="flex min-w-0 items-center gap-1 text-[10px] text-muted @min-[300px]/task:ml-1 @min-[300px]/task:shrink-0 @min-[300px]/task:gap-1.5 @min-[300px]/task:text-[11px]"
      title={`コンテキスト使用量: ${usedLabel} / ${limitLabel} トークン（${pctLabel}）`}
    >
      <span className="h-1 w-6 shrink-0 overflow-hidden rounded-full bg-surface-2 @min-[300px]/task:h-1.5 @min-[300px]/task:w-10">
        <span
          className={cx(
...
export const TaskView = memo(function TaskView({
  ...
  return (
    ...
    >
      <header
        className="grid min-h-11 shrink-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-x-2 border-b border-bot-outline bg-bot-chat px-3 pb-0.5 @min-[300px]/task:px-4"
      ...
      >
            ...
            {titleEditing ? (
              ...
              >
                <input
                  ...
                  className="h-11 min-w-0 flex-1 rounded-lg border border-border-strong bg-bg px-2 text-base font-semibold text-text outline-none focus:border-accent @min-[300px]/task:h-8 @min-[300px]/task:text-sm"
                  disabled={titleBusy}
                />
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-11 w-11 @min-[300px]/task:h-8 @min-[300px]/task:w-8"
                  type="submit"
                  aria-label="タイトルを保存"
                  title="タイトルを保存"
                  busy={titleBusy}
              ...
              <h1
                className={cx(
                  "min-w-0 max-w-full rounded-lg text-left text-sm font-semibold @min-[300px]/task:flex @min-[300px]/task:min-h-8 @min-[300px]/task:items-center @min-[300px]/task:gap-2",
              ...
              >
                {task?.label && (
                  <span className="hidden shrink-0 @min-[300px]/task:inline-flex">
                    <SessionLabelBadge labelId={task.label} className="font-normal" />
                  </span>
                )}
                <span className="block min-w-0 max-w-full truncate leading-5 @min-[300px]/task:flex-1">{task?.title ?? "読み込み中…"}</span>
              </h1>
            )}
            <div aria-label="セッション情報" className="flex h-4 min-w-0 items-center gap-2 overflow-hidden text-[10px] text-muted @min-[300px]/task:hidden">
              <SessionLabelBadge labelId={task?.label} className="font-normal" />
              {contextUsage && (
                <span className="min-w-0 @min-[300px]/task:hidden">
                  <ContextUsageMeter usage={contextUsage} />
                </span>
              )}
            </div>
          </div>
          <div className="hidden @min-[300px]/task:flex">
            <Button
              variant="ghost"
              size="icon"
              aria-label="タイトルを生成"
        ...
        <div aria-label="タスクの状態" className="col-span-1 col-start-1 row-start-2 flex min-w-0 items-center gap-x-2 overflow-hidden text-xs text-muted @max-[300px]/task:-translate-y-0.5">
          ...
          {contextUsage && (
            <span className="hidden @min-[300px]/task:flex">
              <ContextUsageMeter usage={contextUsage} />
            </span>
          )}
          {(stats.totalInputTokens > 0 || stats.totalOutputTokens > 0) && (
            <span
              className="hidden font-mono tabular-nums @min-[300px]/task:inline"
        ...
        >
          {onAddPane && (
            <Button
              ...
              className="h-11 w-11 @min-[300px]/task:h-9 @min-[300px]/task:w-9 @max-[300px]/task:hidden"
              onClick={onAddPane}
            >
          ...
          {compactionAction !== "auto" && (
            <Button
              ...
              className="h-11 w-11 @min-[300px]/task:h-9 @min-[300px]/task:w-9"
              onClick={() => void compact()}
            >
              {!compacting && <Shrink className="h-4 w-4" />}
            </Button>
          )}
          <div className="@min-[300px]/task:hidden">
            <Button
              variant="ghost"
              size="icon"
              aria-label="タイトルを生成"
          ...
          {canManageSupervisor && (
            <label
              ...
              className={cx(
                "relative flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-muted hover:bg-surface-2 hover:text-text focus-within:ring-2 focus-within:ring-accent @min-[300px]/task:h-9 @min-[300px]/task:w-9",
          ...
          {ttsGlobalEnabled && (
            <>
              <button
                ...
                onClick={toggleTts}
                className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg @min-[300px]/task:h-9 @min-[300px]/task:w-9 ${ttsEnabled ? "text-accent" : "text-muted"} hover:bg-surface-2 hover:text-text`}
              >
                {ttsEnabled ? <Volume2 className="h-4 w-4" /> : <VolumeX className="h-4 w-4" />}
              </button>
              {ttsError && <span role="alert" title={ttsError} className="max-w-24 shrink-0 truncate text-[11px] text-danger @min-[300px]/task:max-w-40">{ttsError}</span>}
            </>
          )}
          ...
          <Button
            ...
            disabled={!task}
            className={cx(
              "h-11 w-11 @min-[300px]/task:h-9 @min-[300px]/task:w-9",
