    ...
    <div className="shrink-0 bg-bot-chat px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2 sm:px-4">
      <div className="bot-composer-shell mx-auto w-full max-w-5xl rounded-3xl border border-bot-outline/70 bg-bot-panel px-2 py-1 transition-[border-color,box-shadow] focus-within:border-bot-outline">
        ...
        <div className="flex items-end gap-2">
          ...
          {footer && <button type="button" hidden aria-label="会話のオプション" aria-expanded={optionsOpen} aria-controls={optionsId} onClick={() => setOptionsOpen((open) => !open)} className={`${COMPOSER_ACTION_BUTTON_CLASS} mb-1 bg-surface-2 text-muted hover:bg-surface-3 hover:text-text`}><SlidersHorizontal className="h-4 w-4" /></button>}
            ...
            {showSuggestions && currentToken && (
              <div id={referenceOptionsId} role="listbox" aria-label={currentToken.kind === "skill" ? "スキル候補" : currentToken.kind === "agent" ? "エージェント候補" : "送信プロンプト候補"} className="absolute bottom-full left-0 z-30 mb-2 max-h-56 w-full min-w-64 overflow-y-auto rounded-2xl border border-border bg-surface p-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
                {suggestions.map((reference, index) => {
                  ...
                  return <button key={`${currentToken.kind}-${index}-${reference.name}`} type="button" role="option" aria-selected={selected} onMouseDown={(event) => event.preventDefault()} onClick={() => chooseSuggestion(reference)} className={`flex w-full items-start gap-2 rounded-xl px-3 py-2 text-left ${selected ? "bg-surface-2" : "hover:bg-surface-2"}`}>
