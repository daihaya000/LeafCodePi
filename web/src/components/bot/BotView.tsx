      ...
      {settingsOpen && (
        <aside id="bot-settings-panel" role="dialog" aria-labelledby="bot-settings-title" onKeyDown={(event) => { if (event.key === "Escape") updateSettingsOpen(false); }} aria-label="ボット設定" className="flex h-full w-full shrink-0 flex-col border-bot-outline bg-bot-chat lg:w-[22rem] lg:border-l xl:w-[24.5rem]">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
            ...
            <section className="space-y-2 rounded-2xl border border-border bg-bg p-4" aria-label="個別ツール設定"><span className="text-sm font-medium">使用するツール</span><div className="grid grid-cols-2 gap-x-3 gap-y-1">{BOT_TOOL_NAMES.map((tool) => <label key={tool} className="flex min-w-0 items-center gap-2 text-xs"><input type="checkbox" aria-label={toolNameLabel(tool)} checked={(bot.tools ?? BOT_DEFAULT_TOOL_NAMES).includes(tool)} disabled={updatingTools} onChange={(event) => { const current = new Set(bot.tools ?? BOT_DEFAULT_TOOL_NAMES); if (event.target.checked) current.add(tool); else current.delete(tool); void updateTools([...current]); }} /><span className="truncate" title={tool}>{toolNameLabel(tool)}</span></label>)}</div><p className="text-[11px] text-muted">チェックを外したツールはBotから利用できません。</p></section><p className="text-right text-xs text-muted" role="status" aria-live="polite">{savingProfile ? "保存中…" : "変更は自動保存されます"}</p>
