...
it("shows a routine's consecutive failures and keeps an auto-disabled routine from running", async () => {
  ...
  const permissionSelect = screen.getByRole<HTMLSelectElement>("combobox", { name: "ツール権限" });
  expect(routineRegion.compareDocumentPosition(permissionSelect)).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
...
it("ignores a stale TTS voice result after switching ids", async () => {
  const botOne = { ...testBot, id: "one", name: "One", ttsVoice: "871574624" };
  const botTwo = { ...testBot, id: "two", name: "Two", ttsVoice: "1257529344" };
  let resolvePatch!: (result: { bot: typeof botOne }) => void;
  const patchRequest = new Promise<{ bot: typeof botOne }>((resolve) => { resolvePatch = resolve; });
  let resolveTwo!: (result: { bot: typeof botTwo }) => void;
  const twoResponse = new Promise<{ bot: typeof botTwo }>((resolve) => { resolveTwo = resolve; });
  mocks.getJson.mockImplementation((url: string) => {
    ...
    if (url === "/api/models") return Promise.resolve({ models: [] });
    if (url.endsWith("/routines")) return Promise.resolve({ routines: [] });
    if (url === "/api/settings/tts") return Promise.resolve({ enabled: true, voice: "871574624", rate: 10, url: "http://127.0.0.1:10101" });
    return Promise.resolve({ bot: botOne });
  });
  mocks.sendJson.mockImplementation((url: string) => url === "/api/bots/one" ? patchRequest : Promise.resolve({ bot: botTwo }));
  ...
  await act(async () => {
    resolvePatch({ bot: { ...botOne, ttsVoice: "1257529344" } });
    await patchRequest;
  });

  expect(screen.getByRole("heading", { name: "Two" })).toBeTruthy();
  fireEvent.click(screen.getByRole("button", { name: "設定" }));
  const currentSelect = await screen.findByRole("combobox", { name: "ボットのTTS音声" });
  expect((currentSelect as HTMLSelectElement).value).toBe("1257529344");
});

it("ignores a stale Code auto-approve result after switching ids", async () => {
  const botOne = { ...testBot, id: "one", name: "One", codeAutoApprove: false };
  const botTwo = { ...testBot, id: "two", name: "Two", codeAutoApprove: true };
  let resolvePatch!: (result: { bot: typeof botOne }) => void;
  const patchRequest = new Promise<{ bot: typeof botOne }>((resolve) => { resolvePatch = resolve; });
  let resolveTwo!: (result: { bot: typeof botTwo }) => void;
  const twoResponse = new Promise<{ bot: typeof botTwo }>((resolve) => { resolveTwo = resolve; });
  ...
  await screen.findByRole("heading", { name: "One" });
  fireEvent.click(screen.getByRole("button", { name: "設定" }));
  fireEvent.click(await screen.findByRole("switch", { name: "Codeを常に許可" }));
  await waitFor(() => expect(mocks.sendJson).toHaveBeenCalledWith(
    "/api/bots/one",
    { codeAutoApprove: true },
    "PATCH",
  ...
  await act(async () => {
    resolvePatch({ bot: botOne });
    await patchRequest;
  });

  expect(screen.getByRole("heading", { name: "Two" })).toBeTruthy();
  fireEvent.click(screen.getByRole("button", { name: "設定" }));
  expect((await screen.findByRole("switch", { name: "Codeを常に許可" })).getAttribute("aria-checked")).toBe("true");
});

it("ignores a stale permission mode result after switching ids", async () => {
  const botOne = { ...testBot, id: "one", name: "One", permissionMode: "allow" as const };
  const botTwo = { ...testBot, id: "two", name: "Two", permissionMode: "deny" as const };
  let resolvePatch!: (result: { bot: typeof botOne }) => void;
  const patchRequest = new Promise<{ bot: typeof botOne }>((resolve) => { resolvePatch = resolve; });
  let resolveTwo!: (result: { bot: typeof botTwo }) => void;
  const twoResponse = new Promise<{ bot: typeof botTwo }>((resolve) => { resolveTwo = resolve; });
  ...
  await screen.findByRole("heading", { name: "One" });
  fireEvent.click(screen.getByRole("button", { name: "設定" }));
  const select = await screen.findByRole("combobox", { name: "ツール権限" });
  ...
  expect(screen.getByRole("heading", { name: "Two" })).toBeTruthy();
  fireEvent.click(screen.getByRole("button", { name: "設定" }));
  expect((await screen.findByRole("combobox", { name: "ツール権限" }) as HTMLSelectElement).value).toBe("deny");
});
...
it("keeps the first tool header on the log frame and puts a mixed reply after the log", async () => {
  ...
  const logs = container.querySelectorAll<HTMLDetailsElement>("[data-bot-tool-group]");
  expect(logs).toHaveLength(1);
  expect(logs[0]).toBe(log);
  expect(log.open).toBe(true);
  expect(log.querySelector("summary")?.textContent).toContain("2件");
  expect(log.querySelectorAll("time")).toHaveLength(0);
  expect(log.parentElement?.querySelectorAll("time")).toHaveLength(1);
  const bubble = screen.getByText("Finished reply").closest(".bot-message-bubble")!;
  expect(log.contains(bubble)).toBe(false);
  expect(log.compareDocumentPosition(bubble) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  expect(bubble.previousElementSibling?.querySelectorAll("time")).toHaveLength(1);
});
