...
type HeaderMember = { id: string; name: string; avatarColor?: string; avatarShape?: BotAvatarShape; avatarImage?: string | null; active?: boolean; attention?: boolean };
...
export function BotChatHeader({
  ...
  title: string;
  subtitle: string;
  bot?: { name: string; avatarColor?: string; avatarShape?: BotAvatarShape; avatarImage?: string | null };
  ...
  return (
    <header className="flex h-[3.75rem] shrink-0 items-center gap-2 border-b border-bot-outline bg-bot-chat px-4">
      ...
      <button type="button" onClick={onSettings} aria-label={settingsOpen ? "設定を閉じる" : "設定を開く"} aria-expanded={settingsOpen} aria-controls="bot-settings-panel" className="shrink-0 rounded-full hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
        {bot ? <BotAvatar size={32} color={bot.avatarColor} shape={bot.avatarShape} image={bot.avatarImage} name={bot.name} active={active} /> : <span className="flex h-8 w-8 items-center justify-center rounded-full bg-success-bg text-success"><Users className="h-4 w-4" /></span>}
      </button>
      ...
      {members.length > 0 && <div className="flex shrink-0 -space-x-2" aria-label={`メンバー ${members.length}人`}>
        {visibleMembers.map((member) => <BotAvatar key={member.id} size={26} color={member.avatarColor} shape={member.avatarShape} image={member.avatarImage} name={member.name} active={member.active} className={`ring-2 ${member.attention ? "ring-warning" : "ring-surface"}`} />)}
