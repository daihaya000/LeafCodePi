  ...
  return (
    <header className="flex h-[3.75rem] shrink-0 items-center gap-2 border-b border-bot-outline bg-bot-chat px-4">
      ...
      <button type="button" onClick={onSettings} aria-label="設定を開く" className="shrink-0 rounded-full hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent">
        {bot ? <BotAvatar size={24} color={bot.avatarColor} image={bot.avatarImage} name={bot.name} active={active} /> : <span className="flex h-9 w-9 items-center justify-center rounded-full bg-success-bg text-success"><Users className="h-4 w-4" /></span>}
      </button>
      <button type="button" onClick={onSettings} aria-label="ボット設定を開く" className="min-w-0 flex-1 text-left hover:opacity-80"><h1 className="truncate text-sm font-medium">{title}</h1><p className="sr-only">{subtitle}</p></button>
