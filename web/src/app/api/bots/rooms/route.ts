...
import { createRoom, isRoomNameWithinSize, listRooms } from "@/lib/rooms";
...
export async function POST(req: NextRequest) {
  ...
  if (body?.name !== undefined && (typeof body.name !== "string" || !isRoomNameWithinSize(body.name))) return NextResponse.json({ error: "invalid name" }, { status: 400 });
