...
import { createRoom, listRooms, MAX_ROOM_NAME_CHARS } from "@/lib/rooms";
...
export async function POST(req: NextRequest) {
  ...
  if (body?.name !== undefined && (typeof body.name !== "string" || body.name.length > MAX_ROOM_NAME_CHARS)) return NextResponse.json({ error: "invalid name" }, { status: 400 });
