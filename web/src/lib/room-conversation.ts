...
export function roomBotPrompt(room: RoomDto, bot: BotDto, participants: BotDto[], prompt: string, requestId: string, turn?: RoomTurn): string {
  ...
  return [
    ...
    "For repository work and for repository facts you cannot see from here, use code_session: list projects, request approval, then start an independent Code session for each separate task with an investigate-then-change prompt. When Code must see a screenshot the user attached to this request, pass images as 1-based indexes from availableImages; omit images to attach the latest user message's images, or pass [] to attach none. Code requests run in parallel without a Room queue; coordinate file ownership when work overlaps. To continue a specific session, pass its taskId. A promise to work is not execution. Report only tool-confirmed progress. A starting/running/ready Code record means the Room is waiting for its result; do not duplicate that work or hand it off as completed.",
