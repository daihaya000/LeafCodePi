...
export function isProviderLimitError(error: unknown): boolean {
  ...
  return /^(?:402|429)\b|(?:HTTP(?: status)?|status(?: code)?)\s*[:=]?\s*(?:402|429)\b|GoUsageLimitError|FreeUsageLimitError|usage[_ ]limit(?:[_ ]has[_ ]been)?[_ ](?:reached|exceeded)|hit(?:\s+\S+){0,8}\s+usage[_ ]limit|usage_not_included|monthly usage limit reached|available balance|insufficient[_ ]quota|out of budget|quota exceeded|billing|rate[ ._-]?limit|too many requests/i.test(
    messageOf(error),
  );
}
