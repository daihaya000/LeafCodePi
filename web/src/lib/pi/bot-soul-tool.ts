...
export function validateBotSoulContent(content: unknown): string | null {
  if (typeof content !== "string") return "SOUL.md must be text";
  if (Buffer.byteLength(content, "utf8") > BOT_SOUL_MAX_BYTES) {
    return `SOUL.md is too large (maximum ${BOT_SOUL_MAX_BYTES} bytes)`;
  }
  if (content.includes("\0")) return "SOUL.md cannot contain null bytes";
  return null;
}
  ...
  return (pi) => {
    pi.registerTool({
      ...
      async execute(_toolCallId, input) {
        const validationError = validateBotSoulContent(input.content);
