    ...
    case "clearPane": {
      ...
      if (tabs.length === 0) {
        if (state.panes.length <= 1) {
          return {
            ...state,
            panes: [{ ...target, tabs: [], activeTabId: null }],
          };
        }
        return removePane(state, target.id);
      }
        ...
        panes: state.panes.map((pane) =>
          pane.id === target.id
            ? {
                ...pane,
                tabs,
                activeTabId: tabs.includes(pane.activeTabId ?? "")
                  ? pane.activeTabId
                  : (tabs[0] ?? null),
              }
            : pane,
 ...
 * 新規セッション・Bot画面と Home は、ペイン上限までは新しいペインで開く。新規セッションを
...
export function retargetActiveTab(
  ...
  const sessionTab = urlTaskId !== HOME_TAB_ID && urlTaskId !== SETTINGS_TAB_ID;
