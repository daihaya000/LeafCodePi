...
function parseConfig(id: string): BotConfig | null {
  try {
    ...
    const config: BotConfig = {
      id, name: value.name, label: typeof value.label === "string" ? value.label : "1:1 アシスタント", avatarColor, avatarImage, createdAt: String(value.createdAt), updatedAt: String(value.updatedAt),
...
export function createBot(input: { name?: string; model?: string | null; thinkingLevel?: ThinkingLevel | null; permissionMode?: BotConfig["permissionMode"] }): BotDto {
  ...
  const config: BotConfig = { id, name, label: "1:1 アシスタント", avatarColor: randomAvatarColor(), avatarImage: null, createdAt: now, updatedAt: now, model: input.model ?? null, thinkingLevel: input.thinkingLevel ?? null, permissionMode: input.permissionMode ?? null, skills: { ...DEFAULT_SKILLS }, extraRoots: [], enabled: true, notificationsEnabled: true, codeSessionTaskId: null };
...
export function patchBot(id: string, patch: Partial<Pick<BotConfig, "name" | "label" | "avatarColor" | "avatarImage" | "model" | "thinkingLevel" | "permissionMode" | "skills" | "extraRoots" | "enabled" | "notificationsEnabled" | "codeSessionTaskId">> & { soul?: string }): BotDto | undefined {
