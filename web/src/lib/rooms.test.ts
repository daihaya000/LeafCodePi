...
describe("room store and mention routing", () => {
  ...
  it("defaults botRelayEnabled off and blocks envelopes until enabled", () => {
    ...
    expect(issueRoomRelayEnvelope(room.id, source.id, [target.id])).toBeUndefined();
    patchRoom(room.id, { botRelayEnabled: true });
    expect(issueRoomRelayEnvelope(room.id, source.id, [target.id])).toBeDefined();
  });
