...
「事故 0」は、**LeafCodePi が起動したセッションが strict mode の標準ツールを使う場合**に、次の不変条件で保証する。

1. `write` / `edit` は、実行セッションが有効な file lease を持つパスだけに許可する。
2. 標準 `bash` からのファイル変更、Git の状態変更、commit は許可しない。
...
`leafcode-collaboration` は LeafCodePi の必須 bundled extension とする。無効化できる状態で「write gate がある」と表示するのは危険なので、`WEBUI_REQUIRED_EXTENSIONS` に追加する。
...
保存するのは room のハッシュ、状態 snapshot、短い activity feed、未配信 message だけで、API key・プロンプト全文・ファイル内容は保存しない。
...
- **event log**: 直近の bounded feed のみ。無制限に履歴を積まない
...
| write / edit | 拒否 |
| `leafcode_run` | read-only check だけ許可 |
| `leafcode_commit` | 拒否 |
| reserve / release | 拒否 |

古い lease は自動で clean 扱いにしない。未 commit の lease は `orphaned` として残し、所有セッションの再接続または明示的な user takeover が必要になる。
...
`tool_call` handler は次の順に検査する。

1. room client が connected か
2. path が canonical repository-relative path か
3. current epoch / fencing token が有効か
4. current session の lease selector に一致するか
5. 他セッションの lease と重ならないか
6. 直前に観測した fingerprint から外部変更されていないか

一つでも失敗したら `{ block: true, reason }` を返す。成功した write の `tool_result` 後に fingerprint を再計算し、`observed` と activity feed を更新する。tool が失敗した場合はファイルを再スキャンし、予期しない変更があれば lease を `invalid` にする。
...
- `send`: target の inbox に配信。通常は target の次の turn で表示し、勝手に turn を起動しない
- `ask`: reply を要求する message。target が idle なら一度だけ turn を起動し、最大 timeout 後に失敗する
...
message は peer からの**未信頼入力**として扱う。message の内容で system policy、permission、lease 判定を変更しない。
...
strict mode では標準 `bash` を**全面的に拒否**する。read-only に見える command でも `find -delete`、`git diff --output`、alias、pager、`--exec` などの抜け道を安全に判定できないため、正規表現の allowlist は作らない。探索は Pi の `read` / `grep` / `find` / `ls`、検証は `leafcode_run` を使う。

標準 `bash` に対する次の拒否は常に同じエラーで返す。

```text
git add / commit / reset / restore / checkout / switch / merge / rebase /
stash / clean / push、redirect、pipe、subshell、任意 script write
```

将来 read-only shell の fast path を追加する場合も、正規表現ではなく argv parser と command ごとの option schema を使い、Phase 3 の受け入れ条件を再実行する。

### 8.2 `leafcode_run`

テスト・lint・build は `leafcode_run({ command, purpose: "check" })` で実行する。

- `git` の状態変更サブコマンドは常に拒否する
- `git` を呼び出す command は既定で拒否する。project config に追加する場合も、変更を行わない検証 command として明示登録する
- 実行前後に repository fingerprint を取得する
- lease 内の変更は owner の observed に反映する
- lease 外の変更、他セッションの lease への変更、追跡ファイルの予期しない変更は `foreign-change` として全 commit を停止する
- 外部コマンドが変更したファイルを自動で owner にしない

Node の child process で arbitrary shell を完全に sandbox するものではない。そのため run は「変更を防ぐ機能」ではなく「変更を commit に混ぜない機能」として設計し、実行後検査を必須にする。
...
and reserve the exact files. Never use bash for writes or git state changes.
Use leafcode_run for checks and leafcode_commit for commits with explicit paths.
...
- `tool_call`: write/edit/bash gate、current tool 更新
...
| `web/src/lib/extensions.ts` | bundled extension の必須登録 |
| `web/src/lib/pi/harness.ts` | bundled extension と strict policy の起動確認。session ごとの状態を WebUI へ渡す場合のみ追加 |
| `README.md` | reserve / run / commit の利用規約と保証範囲 |
...
既存 `leafcode-subagents` の native supervisor channel は変更しない。subagent child も room には参加するが、parent の lease を暗黙継承しない。parent が child に書き込みを委任する場合は child session ID への明示 grant を追加する。
...
`mode: permissive` は開発用に残しても、LeafCodePi の既定値にはしない。permissive では事故 0 を主張できないため、WebUI の extension status に明確な警告を出す。

## 12. 実装フェーズ

### Phase 1: room / presence / communication

- canonical project key、coordinator election、Named Pipe / Unix socket
- join、heartbeat、list、feed、send、ask / reply
- TUI の status / widget
- broker 復旧と stale epoch のテスト

### Phase 2: lease / write gate

- reserve、renew、release、claim
- `write` / `edit` の strict gate
- lease fingerprint と foreign-change 検出
- 2 セッションの同一 path conflict テスト

### Phase 3: run / commit gate

- bash read-only classifier
- `leafcode_run` の before / after scan
...
Phase 1〜3 で「並列編集事故を防ぐ」機能は成立する。WebUI は可観測性の追加であり、gate の判断経路にはしない。

## 13. 受け入れ条件

1. 同一 project の 2 セッションが 1 秒以内に同じ room の `list` に現れる
2. active / idle / away / stuck と goal / model / branch / current tool が相互に見える
3. A が `src/a.ts` を reserve 中、B の write/edit が tool level で拒否される
4. lease なしの write/edit が拒否される
5. standard bash の `git commit`、redirect、任意 script write が拒否される
...
11. `npm --prefix web run typecheck` と対象 Vitest が成功する
...
最小の安全な形は、**共有 room + 低遅延 IPC + hard lease + 専用 commit transaction** である。presence や DM だけを先に作ると「見えているが防げない」ため、Phase 2 と Phase 3 を省略しない。
